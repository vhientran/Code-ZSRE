...
def main():
    ...
    parser.add_argument('--pid2name', default='pid2name_wiki',
                        help='pid2name_wiki file: relation names and description on wiki-zsl set')
    ...
    test_relations = file_relations['wiki'][str(opt.id)][str(opt.m)]
    ...
    if not opt.only_test:
        ...
        framework.train(model, N, learning_rate=opt.lr, weight_decay=opt.weight_decay,
                        alpha=opt.alpha, train_iter=opt.train_iter,
                        val_step=opt.val_step, grad_iter=opt.grad_iter)
        T2 = time.clock()
