...
class WikiZSLDataset(data.Dataset):
    """
    Wiki-ZSL Dataset
    """
    ...
    def __init__(self, pid2name, encoder, N, root, test_relations=None):
        ...
        path = os.path.join(root, "wiki_train_new.json")
        ...
        json_train_data = {} # save train data

        # remove relations in test set, to prepare the train set
        for g in json_full_data:
            relation_id = g['edgeSet'][0]['kbID']
            if relation_id in test_relations:
                continue
            if relation_id not in json_train_data:
                json_train_data[relation_id] = []
            # add item to dict
            edge = g["edgeSet"][0]
            pos_ent1, pos_ent2 = edge['left'], edge['right']
            if len(pos_ent1)==0 or len(pos_ent2)==0:
                continue
            dict_sent = {}
            dict_sent["tokens"] = g["tokens"]
            dict_sent["pos_ent1"] = pos_ent1
            dict_sent["pos_ent2"] = pos_ent2
            # add dict_sent to json_train_data
            json_train_data[relation_id].append(dict_sent)
...
def get_loader(pid2name, encoder, N,
    ...
    dataset = WikiZSLDataset(pid2name, encoder, N, root, test_relations)
...
def get_test_loader(pid2name, encoder, batch_size=32, test_relations=None, root='../official_data'):
    ...
    for g in json_full_data:
        relation_id = g['edgeSet'][0]['kbID']
        if relation_id not in test_relations:
            continue
        if relation_id not in json_test_data:
            json_test_data[relation_id] = []
        ...
        dict_sent["pos_ent2"] = pos_ent2
        # add dict_sent to json_train_data
        json_test_data[relation_id].append(dict_sent)
    ...
    for i,id_label in enumerate(ids_classes):
        # get the name and relation descp of the relation
